<template>
        <el-container>
                ...
                <el-aside width="250px" class="m-layer-aside">
                        ...
                        <el-input type="text" placeholder="标题" v-model="sctitle" size="small" style="margin:10px 10px 10px 0;width:200px;">
                        </el-input>
                        <div class="m-title" style="border-color:transparent;">图层设置</div>
                        <el-checkbox-group v-model="selectedLayers">
                                <el-checkbox v-for="item in sclayers" style="width:50%;margin-left:0;" :key="item.code" :label="item.code">{{item.name}}</el-checkbox>
                        </el-checkbox-group>
                        ...
                        <div class="m-title" style="border-color:transparent;">色带设置</div>
                        <div class="m-symbols-color" v-if="showColor">
                                <ul v-if="rgbVo">
                                        <li v-for="(item,index) in rgbVo.rgb" :style="'background:#'+item" @click="changeSymbols" :data-index="index"></li>
                                </ul>
                        </div>
                        <div class="m-symbols">
                                <ul v-if="scsymbols">
        ...
        export default {
                ...
                methods: {
                        ...
                        getSuferChart() {
                                ...
                                axios.get(tool.remoteUrl + "/" + keyTable+ "/executeSuferChart.do?" + str + "chartInfoVo.interpolation=" + _this.selectedInterpolation +
