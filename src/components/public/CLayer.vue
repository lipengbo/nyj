  ...
  export default {
    ...
    watch: {
      ...
      selectedYear(val){
        ...
        var stData=this.query.statYearData.statisticsList;
        var minData=null;
        var maxData=null;
        ...
        for(let i=0;i<minData.length;i++){
          if(minData[i].text.indexOf(this.selectedYear+"")!=-1){
            this.scminvalue=minData[i].value;
            break;
          }
        }
        for(let i=0;i<maxData.length;i++){
          if(maxData[i].text.indexOf(this.selectedYear+"")!=-1){
            this.scmaxvalue=maxData[i].value;
            break;
          }
        }
    ...
    created() {
      this.years=[];
      for(let i=this.query.syear;i<=this.query.eyear;i++){
        this.years.push(i)
      }
    ...
    methods: {
      ...
      async getSuferChart() {
       var queryStr =
          'climateQueryVo.syear=' + this.query.syear + "&" +
