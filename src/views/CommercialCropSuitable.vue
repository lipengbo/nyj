    ...
    <div class="m-stat-header">
      <div class="u-bar u-white f-bb1 " style="height:60px;line-height:60px;">
        <ul v-if="eles" class="m-eles">
          <label for="m-element" class="u-label">特色作物 ：</label>
...
<script>
  ...
  export default {
    name: "commercialcropsuitable", //特色作物
    data() {
      return {
        eles: null,
        selectedEle: null,
    ...
    methods: {
      getOptions() {
        var axios = this.$axios,
          _this = this;
        axios.get(baseUrl + "/getAgrForecastEleVoListByType.do?type=CommercialCropSuitable").then(res => {
          _this.eles = res.data;
