                ...
                <div class="m-stat-header">
                        <div class="u-bar u-white f-bb1 " style="height:60px;line-height:60px;">
                                <ul v-if="eles" class="m-eles">
                                        ...
                                        <li v-for="(item,index) in eles" :data-index="index" :data-value="item.value" :class="selectedEle==item.value?'f-active':''" @click="changeEle">{{item.text}}</li>
                                </ul>
                        </div>
                        <div class="u-bar u-white f-bb1 ">
                                ...
                                <ul>
                                        ...
                                        <li style="margin-right:0;">
                                                ...
                                                -
                                                <el-date-picker format="yyyy-MM-dd" size="mini" v-model="eDate" type="date" placeholder="选择日期" @change="eDateChange">
                                                </el-date-picker>
                                        </li>
                ...
                methods: {
                        getOptions() {
                                var axios = this.$axios,
                                        _this = this;
                                axios.get(baseUrl + "/getAgrForecastEleVoListByType.do?type=Diseasesuitable").then(res => {
                                        _this.eles = res.data;
                        ...
                        getAgrForecastImageVosByQueryVo() {
                                ...
                                var url = baseUrl + "/getAgrForecastImageVosByQueryVo.do?queryVo.eletype=" + _this.selectedEle + "&queryVo.enddate=" + _this.eDate +
                                        "&queryVo.orgcode=" + _this.orgInfo.code + "&queryVo.startdate=" + _this.sDate + "&queryVo.stationtype=" + _this.selectedStationType +
                                        "&queryVo.type=Diseasesuitable";
                        ...
                        getAgrForecastInfoStatisticsVoByQueryVo() {
                                ...
                                var url = baseUrl + "/getAgrForecastInfoStatisticsVoByQueryVo.do?queryVo.eletype=" + _this.selectedEle + "&queryVo.enddate=" + _this.eDate +
                                        "&queryVo.orgcode=" + _this.orgInfo.code + "&queryVo.startdate=" + _this.sDate + "&queryVo.stationtype=" +
                                        _this.selectedStationType + "&queryVo.type=Diseasesuitable";
                                var tableList = [],
                                        tableColumns = [{
                                                value: "stacode",
                                                text: "站点"
